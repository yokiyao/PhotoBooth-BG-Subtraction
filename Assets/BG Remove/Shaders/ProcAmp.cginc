...
half4 ProcAmp(float2 uv)
{
	...
	//my second texture rgb data
	half4 src2 = tex2D(_BGTex, uv);
	half3 rgb2 = src2.rgb;
    ...
    rgb = GammaToLinearSpace(rgb);
    #endif
																	//use the alpha to switch the two textures
	half3 mixrgb = lerp(rgb, rgb2, 1-alpha);

    return half4(mixrgb, /*alpha*/ 1 * _Opacity) * mask;                           
