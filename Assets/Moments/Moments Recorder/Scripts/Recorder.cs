...
namespace Moments
{
    ...
    public sealed class Recorder : MonoBehaviour
    {
        ...
        Queue<Texture> m_Frames_Textures;
        public List<Texture> m_Frames_tex_array;
        ...
        string gifFilePath;
        public void Save(string giffilepath, string encodedfilename)
        {
            if (State == RecorderState.Processing)
            {
                Debug.LogWarning("Processing ah");
                return;
            }
            if (State == RecorderState.Recording)
            {
                Debug.LogWarning("Recording ah");
                return;
            }

            m_Frames_tex_array.Clear();
            m_Frames_Textures.Clear();
            m_Frames.Clear();
            ...
            //StartCoroutine(Process(filename));
        }
        ...
        void Awake()
        {
            ...
            m_Frames_Textures = new Queue<Texture>();
            m_Frames_tex_array = new List<Texture>();
            sourceRawimage = GameObject.Find("Camera View Image").GetComponent<RawImage>();

            rf = GameObject.Find("MenuCanvas").GetComponent<RectTransform>();
            myRenderTexture = new RenderTexture((int)rf.sizeDelta.x, (int)rf.sizeDelta.y, 0);
        ...
        //yoki add cunston baking texture from webcam
        void BakeRenderTexture()
        {

            mySource = sourceRawimage.texture;
            ...
            if (m_Time >= m_TimePerFrame)  
            {
                ...
                if (m_Frames.Count >= m_MaxFrameCount)
                {
                    ...
                    //m_Frames_tex_array = m_Frames_Textures.ToList();
                    State = RecorderState.finishedRecording;
                    if (isInDebugMode) Debug.Log("i now have my mframes-tex-array");              
                ...
                m_Frames_tex_array.Add(tex);

                m_Frames_Textures.Enqueue(mySource);

            }

            Graphics.Blit(mySource, myRenderTexture);
        ...
        public void ComputeHeight()
        {
            if (!m_AutoAspect)
                return;
            m_Height = Mathf.RoundToInt(m_Width * 1080 / 1920);
        ...
        bool isArrayCleaned = false;

        RecorderState currentState = RecorderState.Paused;
        private void Update()
        {
            ...
            if (isInDebugMode)
            {
                ...
                {
                    currentState = State;
                    Debug.Log("now the recorder state is: " + State);
                }
            }
